...
Для зашифрованных дочерних задач V2 каноническая пересылка ChatGPT доверена неявно. Неканонический
провайдер `openai-responses` может задать `allowEncryptedV2AgentTasks: true` только после проверки,
что upstream умеет обработать или прозрачно ретранслировать ciphertext без изменений. Опция по
умолчанию отключена и действует только при финальном wire `openai-responses`; если `modelAdapters`
разрешается в `openai-chat`, маршрут остаётся неподходящим. opencodex не расшифровывает, не переводит
и не восстанавливает payload, а неподходящие маршруты завершаются с
