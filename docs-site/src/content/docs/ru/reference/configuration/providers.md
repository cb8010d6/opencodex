...
| `allowEncryptedV2AgentTasks?` | `boolean` | По умолчанию выключено. Доверяет этому неканоническому провайдеру `openai-responses` обработку непрозрачных зашифрованных задач подагентов V2. При включении opencodex передаёт `encrypted_content` без изменений и пропускает восстановление plaintext для этого маршрута. Это не доказывает совместимость и не даёт способности расшифровывать; включайте только после проверки upstream. Канонической пересылке ChatGPT флаг не нужен. |
