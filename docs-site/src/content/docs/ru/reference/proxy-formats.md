...
| 400 | `unreadable_encrypted_agent_task` | У шифрованной задачи воркера v2 нет подходящего канонического потребителя ChatGPT или явно разрешённой Responses-цели, у которой итоговый wire выбранной модели остаётся `openai-responses` и которая способна прозрачно обработать или передать ciphertext |
...
разрешённой Responses-цели с итоговым wire `openai-responses` нет, opencodex завершит запрос ошибкой `unreadable_encrypted_agent_task`, вместо того чтобы
