...
| `allowEncryptedV2AgentTasks?` | `boolean` | Désactivé par défaut. Fait confiance à ce fournisseur `openai-responses` non canonique pour traiter les tâches de sous-agent V2 chiffrées et opaques. Une fois activé, opencodex transmet `encrypted_content` sans modification et ignore la récupération en texte brut pour cette route. Cette option ne prouve pas la compatibilité ni la capacité de déchiffrement ; ne l’activez qu’après vérification du service en amont. Le transfert ChatGPT canonique n’en a pas besoin. |
