...
| 400 | `unreadable_encrypted_agent_task` | 加密的 v2 worker task 沒有能消耗它或不透明轉發它的合格規範 ChatGPT 消費者，或選定模型的最終 wire 保持為 `openai-responses` 且明確受信任、能消耗或不透明轉發它的 Responses 目標 |
