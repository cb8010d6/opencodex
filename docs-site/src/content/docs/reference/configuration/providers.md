...
| `allowEncryptedV2AgentTasks?` | `boolean` | Disabled by default. Trust this non-canonical `openai-responses` provider to consume opaque encrypted V2 sub-agent tasks. When enabled, opencodex passes matching `encrypted_content` through unchanged and skips plaintext recovery for that route. It does not prove compatibility or grant decryption capability; enable only after verifying the upstream. Canonical ChatGPT forwarding is always eligible and does not need this flag. |
